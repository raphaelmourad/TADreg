...
source("script/Other_Lib.R")
source("script/TADregMiscFun.R")
source("script/TADregProcData.R")
source("script/TADregSIM.R")
source("script/TADregDIM.R")
source("script/TADregPredHiC.R")
source("script/TADregPIM.R")
...
for(k in 1:length(cells)){
 # Load Hi-C data
 filek=paste0("data/HiC_Bonev/HiC_",cells[k],"_mouse_chr18_50-70Mb_HTCexp.RData")
 load(filek)
 print(cells[k])
}
...
plot(posPlot/1e6,coefPlot[x1:x2],type="l",xlab="Position (Mb)",ylab="SIM beta")
dev.off()
...
HTC_PredWT=PIM_noSV(HTC=HTC_WT,model=model,distMax=distMax,noise=noise)
